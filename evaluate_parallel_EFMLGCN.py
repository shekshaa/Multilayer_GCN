...
from models import Parallel_EF_ML_GCN
...
def evaluate(dataset):
    ...
    for m, aggr in enumerate(aggregations):
        FLAGS.aggregation = aggr
        val_f1_arr = []
        test_f1_arr = []
        for i, l in enumerate(learning_rates):
            ...
            for j, h1 in enumerate(hidden1):
                FLAGS.hidden1 = h1
                print("Aggregation: " + aggr + ", Learning rate: {}, hidden1: {}".format(l, h1))
        ...
        val_df = pnd.DataFrame(data=val_f1_arr, index=learning_rates, columns=hidden1, dtype=str)
        test_df = pnd.DataFrame(data=test_f1_arr, index=learning_rates, columns=hidden1, dtype=str)
        val_df.to_csv(path_or_buf='./log/Parallel_EFMLGCN/' + time_str + '/' + aggr + '_val_f1.csv')
        test_df.to_csv(path_or_buf='./log/Parallel_EFMLGCN/' + time_str + '/' + aggr + '_test_f1.csv')
