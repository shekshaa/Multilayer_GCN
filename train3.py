...
from models import WeightedMixedAutoencoder
...
flags.DEFINE_float('lmbda', 1., 'Weight for label classification loss term')
...
model = WeightedMixedAutoencoder(name='Multilayer_GCN',
...
train_writer = tf.summary.FileWriter(logdir='./log/MixedAutoencoder/{}/train/'.format(save_path))
val_writer = tf.summary.FileWriter(logdir='./log/MixedAutoencoder/{}/val/'.format(save_path))
...
for epoch in range(FLAGS.epochs):
    ...
    train_summary, label_acc, train_edge_f1, train_loss = sess.run(
        [model.summary1, model.type_acc, model.f1, model.total_loss], feed_dict=feed_dict)
