...
flags.DEFINE_float('lmbda', 0.1, 'Weight for type classification loss term')
flags.DEFINE_integer('early_stopping', 10, 'Tolerance for early stopping (# of epochs).')
flags.DEFINE_integer('max_degree', 3, 'Maximum Chebyshev polynomial degree.')

if FLAGS.dataset == 'infra':
    all_sub_adj, node_types = load_infra()
    ...
    all_sub_adj, node_types = load_aminer()
    train_adj, train_mask, val_mask, test_mask = load_train_val_test2(all_sub_adj)
...
model = Model(name='Multilayer_GCN', placeholders=placeholders, num_nodes=train_adj.shape[0], super_mask=super_mask)
...
for epoch in range(FLAGS.epochs):
    ...
    ____, summary, train_type_acc, train_edge_f1, train_loss = sess.run(
        [model.opt, merged_summary, model.type_acc, model.precision, model.total_loss], feed_dict=feed_dict)
